...
cppcgen makes an attempt to allow for easy structured generation of
C or simple C++ subroutines incorporating some repeated code structures
or similar complex expressions. The expressions definitions can use macroses,
which are predefined at runtime in Expressions Directory. Besides the
strightforward macroses expansion, the macroprocessor also
...
The portability is not a strong goal for now, but code and Makefiles are 
